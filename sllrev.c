...
void Reverse(){
    ...
    prev = NULL;
    while(curr != NULL){
        next = curr ->next; // Before changing the address of the current node to null, we first retrieve the address of the next node
        curr->next = prev;
        prev = curr;   // The current node becomes previous node in the next iteration.
        curr = next;    //This is for iteration puropose
    }
    head = prev;        //head now points to the last node since the loop is exited if the curr is null, the prev will have a proper value.
}
